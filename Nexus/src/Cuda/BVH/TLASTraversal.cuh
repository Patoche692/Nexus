...
inline __device__ void TLASTrace(const D_TLAS& tlas, D_Ray& ray)
{
	...
	while (1)
	{
		if (node->IsLeaf())
		{
			BVHInstanceTrace(tlas.blas[node->blasIdx], tlas.bvhs, ray, node->blasIdx);
...
inline __device__ bool TLASTraceShadow(const D_TLAS& tlas, D_Ray& ray)
{
	D_TLASNode* node = &tlas.nodes[0], * stack[16];
	uint32_t stackPtr = 0;
	...
	while (1)
	{
		if (node->IsLeaf())
		{
			if (BVHInstanceTraceShadow(tlas.blas[node->blasIdx], tlas.bvhs, ray))
				return true;
