...
void MaterialManager::AddMaterial(Material& material)
{
	...
	checkCudaErrors(cudaMalloc((void**)&m_DevicePtr[m.id], sizeof(m)));
...
bool MaterialManager::SendDataToDevice()
{
	...
	{
		invalid = true;
		checkCudaErrors(cudaMemcpy((void*)m_DevicePtr[id], (void*)&m_Materials[id], sizeof(m_Materials[id]), cudaMemcpyHostToDevice));
	}
